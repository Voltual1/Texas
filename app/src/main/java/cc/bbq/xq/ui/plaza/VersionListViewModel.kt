...
class VersionListViewModel(
    ...
    fun loadVersions(packageName: String, store: AppStore) {
        // 避免重复加载相同数据
        if (currentPackageName == packageName && currentStore == store && _versions.value.isNotEmpty()) {
            return
        }
        ...
        viewModelScope.launch {
            try {
                ...
                val result = repository.getAppVersionsByPackageName(packageName)

                if (result.isSuccess) {
                    _versions.value = result.getOrThrow()
                } else {
